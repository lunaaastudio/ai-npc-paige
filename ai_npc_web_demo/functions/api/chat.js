...
// 与 server.mjs 同一套逻辑的无依赖移植（本地规则引擎 + 可选 OpenAI）
// 环境变量（在 Pages 项目 Settings → Environment variables 配置）：
//   OPENAI_API_KEY（可选，不配则走本地规则引擎 demo 模式）
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (!env.OPENAI_API_KEY) {
      ...
      return json({ ...localPaige(last), demo: true });
    }
    ...
    if (!apiRes.ok) {
      ...
      return json({ error: payload?.error?.message || 'OpenAI request failed' }, apiRes.status);
    }
    const raw = extractOutputText(payload) || '';
    const { reply, emotion, notes } = parsePaige(raw);
    return json({ reply: reply || '（Paige 把便签贴好了，没多说什么。）', emotion, notes, demo: false, model });
  } catch (err) {
    console.error(err);
